...
export const activate = async (context: vscode.ExtensionContext) => {
    ...
    mcpServer.tool(
        ...
        dedent`
            ...
            If a breakpointId is provided, the function will wait for execution to pause at that specific breakpoint before returning.
        `.trim(),
        {
            ...
            breakpointId: z
                ...
                .optional()
                .describe(
                    'Optional breakpoint ID to wait for. If provided, the function will only return when execution pauses at this breakpoint.',
                ),
        },
        async (params: { testFilePath: string; testNamePattern?: string; breakpointId?: string }) => {
            try {
                const result = await debugJestTest(params.testFilePath, context, params.testNamePattern, params.breakpointId);
                return {
                    ...result,
                    content: result.content.map((c) => ({
                        ...c,
